B_KWON2.IDX <- function(x, kmax, method = "FCM",
  ...
  for(k in kmin:kmax){
    ...
    kwon2[k-kmin+1] = w1*((w2*sum(d4)) + (sum(d2)/max(d2)) + w3 ) / (min(d3) + 1/k + 1/(k^(fzm - 1)))
  }
  CVI.dframe = data.frame("C" = kmin:kmax,"Index" = kwon2)
  ...
  colnames(CVI.dframe) = c("k","KWON2")
  KWON2.result = list("BCVI" = BCVI,"VAR" = VarBCVI,"Index" = CVI.dframe)
  return(KWON2.result)
}
