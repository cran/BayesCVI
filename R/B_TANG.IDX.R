B_TANG.IDX <- function(x, kmax, method = "FCM",
  ...
  for(k in kmin:kmax){
    ...
    adp = (1/(k*(k-1)))*sum(d3)
    tang[k-kmin+1] = (sum(d1) + 2*adp)/ (min(d3) + 1/k)
  }
  CVI.dframe = data.frame("C" = kmin:kmax,"Index" = tang)
  ...
  colnames(CVI.dframe) = c("k","TANG")
  TANG.result = list("BCVI" = BCVI,"VAR" = VarBCVI,"Index" = CVI.dframe)
  return(TANG.result)
}
