B_WL.IDX <- function(x, kmax, method = "FCM",
  ...
  for(k in kmin:kmax){
    ...
    wl[k-kmin+1] = sum(d1/d6) / (min(d3) + median(d3))
  }
  CVI.dframe = data.frame("C" = kmin:kmax,"Index" = wl)
  ...
  colnames(CVI.dframe) = c("k","WL")
  WL.result = list("BCVI" = BCVI,"VAR" = VarBCVI,"Index" = CVI.dframe)
  return(WL.result)
}
