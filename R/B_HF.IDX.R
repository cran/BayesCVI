B_HF.IDX <- function(x, kmax, method = "FCM",
  ...
  for(k in kmin:kmax){
    ...
    adp = (1/(k*(k-1)))*sum(d3)
    hf[k-kmin+1] =  (sum(d5) + 2*adp)/ ((n/2*k)* (min(d3) + median(d3)))
  }
  CVI.dframe = data.frame("C" = kmin:kmax,"Index" = hf)
  ...
  colnames(CVI.dframe) = c("k","HF")
  HF.result = list("BCVI" = BCVI,"VAR" = VarBCVI,"Index" = CVI.dframe)
  return(HF.result)
}
