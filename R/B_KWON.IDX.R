B_KWON.IDX <- function(x, kmax, method = "FCM",
  ...
  for(k in kmin:kmax){
    ...
    kwon[k-kmin+1] = (sum(d1)+ mean(d2))/min(d3)
  }
  CVI.dframe = data.frame("C" = kmin:kmax,"Index" = kwon)
  ...
  colnames(CVI.dframe) = c("k","KWON")
  KWON.result = list("BCVI" = BCVI,"VAR" = VarBCVI,"Index" = CVI.dframe)
  return(KWON.result)
}
